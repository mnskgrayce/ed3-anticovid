...
def QRcheck():
    ...
    valid = 2                                              # reset value for next loop
    ...
    while stsQRcam and count != STEP_TIME:                 # Frequency 10Hz, 0.1s for 1 count
        ...
        for barcode in decode(img):                        # Scan Barcode
            ...
            if myData in myDataList:                       # Check on the list or not
                ...
                stsQRcam = 0              
                if total < 4:                              # Check people in room less than 5
                    enter = enter + 1
                    relay.on()
                    time.sleep(0.5)
                    relay.off()
        ...
        if count == STEP_TIME:
            if valid == 0:                                 # QR check un-authorized status
                exit1 = exit1                               
            if valid == 2:                                 # QR check authorized status
                exit1 += 1
                if total == 0:                             # nagative value case
                    exit1 = 0
